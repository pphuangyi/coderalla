...
chromedriver_path = '/path_to/chromedriver.exe'

# path to write list of found appointments to
output_path = '/e.g./Dropbox/IFTTT'


###### Can probably leave these as-in

# rough distances per GPS coord near this location
miles_per_lat = 69.1
miles_per_long = 52.93

# NY place lists and GPS coords from:
# https://en.wikipedia.org/wiki/List_of_towns_in_New_York (+ 2 nassau cities /w GPS info)
# https://en.wikipedia.org/wiki/List_of_villages_in_New_York_(state)
# https://en.wikipedia.org/wiki/List_of_census-designated_places_in_New_York
# ...ignore "List_of_cities_in_New_York", only 2 in Nassau, 0 in Suffolk
ny_towns = [t.split('\t') for t in open('ny_towns.tsv').read().splitlines()]
ny_villages = [v.split('\t') for v in open('ny_villages.tsv').read().splitlines()]
ny_places = [p.split('\t') for p in open('ny_places.tsv').read().splitlines()]

# de-duplicate place lists into dictionaries, keep higher population
def deduplicated_dict(original_list):
    ddict = {}
    for item in original_list:
        name = item[0]
        if not name in ddict:
            ddict[name] = item
        else:
            if not ddict[name][2]:
                continue
            
            prev_pop = int(ddict[name][2].replace(',',''))
            curr_pop = int(item[2].replace(',',''))
            if curr_pop > prev_pop:
                ddict[name] = item
    return ddict

# dictionaries of towns, villages, and places
towns = deduplicated_dict(ny_towns)
villages = deduplicated_dict(ny_villages)
places = deduplicated_dict(ny_places)
...
while True:
    ...
    for i, loc in enumerate(locations):
        ...
        if loc[1] == 'Nassau' or loc[1] == 'Suffolk':
            ...
            loc_long = float(loc_coord[1])
            dist_lat = (loc_lat - my_lat) * miles_per_lat
            dist_long = (loc_long - my_long) * miles_per_long
            dist = (dist_lat * dist_lat + dist_long * dist_long) ** (1/2)
